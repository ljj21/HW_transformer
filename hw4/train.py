...
#             train.py - main program for training process
#             Student ID:
#             Name:
#             Tsinghua University
#             (C) Copyright 2023
...
def train(data_root, model_name, batch_size, n_iters, ckpt_path, resume, val_interval, device='cpu', max_seq_len=1024, no_res=False, no_pos=False):
    gradient_accumulation_steps = 1
    train_dataset = LMDataset(data_root, 'train', max_seq_len)
    val_dataset = LMDataset(data_root, 'val', max_seq_len)
    train_loader = DataLoader(train_dataset, batch_size=int(batch_size//gradient_accumulation_steps), shuffle=True)
    ...
    if os.path.exists(meta_path):
        ...
        print("data are not well prepared, please run prepare.py first!")
    ...
    if not resume:
        ...
        pretrained_model_path = os.path.join(ckpt_path, 'pretrain.pth')
        print(f"Initializing from : {pretrained_model_path}")
        ...
        if 'iter_num' in checkpoint:
            pre_iter_num = checkpoint['iter_num']
    ...
    epoch_num = np.ceil(n_iters / float(len(train_dataset))).astype(np.int32)
    t0 = time.time()
    model.train()
    train_losses = []
    val_losses = []
    for epoch in range(epoch_num):
        for step, (X, Y) in enumerate(train_loader):
            if iter_num == 1:
                losses = estimate_loss(model, val_loader, ctx, device)
            ...
            lr = get_lr(iter_num+pre_iter_num, learning_rate, lr_decay_iters=n_iters+pre_iter_num)
            ...
            if (micro_step % gradient_accumulation_steps) == 0:
                ...
                print(f"iter {iter_num+1}: loss {lossf:.4f}, perplexity {np.exp(lossf):.4f}, time {dt*1000:.2f}ms")
                micro_step = 1
                iter_num += 1
                ...
                if iter_num % val_interval == 0:
                    losses = estimate_loss(model, val_loader, ctx, device)
                    val_losses.append(losses['val'])
                    print(f"step {iter_num}: val loss {losses['val']:.4f}, perplexity {np.exp(losses['val']):.4f}")
...
def plot(n_iters, train_losses, val_losses, val_interval, ckpt_path):
    ...
    val_iters = np.arange(0, n_iters+1, val_interval)
...
@torch.no_grad()
def estimate_loss(model, val_loader, ctx, device):
    out = {}
    model.eval()
    losses = 0
    max_iters = 100
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_seq_len', type=int, default=64, help='the maximum number of sequence length')
    parser.add_argument('--iters', type=int, default=2000, help='number of training epochs')
    ...
    parser.add_argument('--resume', action='store_true', help='whether to initialize model with pretrained model')
    ...
    train(opt.data_root, opt.model_name, opt.batchsize, opt.iters, opt.ckpt_path, opt.resume, opt.val_interval, opt.device, opt.max_seq_len, opt.no_res, opt.no_pos)
